...
import { ENABLED_INCOME_SOURCE_IDS, incomeSources } from "@/lib/data";
import { useAppStore } from "@/lib/store";

const enabledIncomeIds = new Set<string>(ENABLED_INCOME_SOURCE_IDS);
...
export default function IncomeSourcesPage() {
  ...
  const canContinue = selected.some((id) => enabledIncomeIds.has(id));
      ...
      <main className="relative z-10 mx-auto w-full max-w-4xl flex-1 px-5 pt-8 pb-16 sm:pt-10 sm:pb-24">
        <div className="text-center">
          <h1 className="text-2xl font-extrabold tracking-tight sm:text-3xl">
            First, select where does your income come from.
          </h1>
        ...
        <div className="mt-8 space-y-3">
          {incomeSources.map((source) => {
            ...
            const isDisabled = !enabledIncomeIds.has(source.id);
            return (
              <button
                key={source.id}
                type="button"
